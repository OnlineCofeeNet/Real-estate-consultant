...
function verifySession(token: string): SessionPayload | null {
  ...
  if (signature.length !== expected.length || !createHmac('sha256', AUTH_SECRET).update(signature).digest('hex').startsWith(createHmac('sha256', AUTH_SECRET).update(expected).digest('hex'))) {
    return null;
  }
  try {
    const payload = JSON.parse(Buffer.from(encoded, 'base64url').toString('utf8')) as SessionPayload;
    if (!payload.userId || !payload.username || !USER_ROLES.has(payload.role) || !payload.exp || payload.exp < Math.floor(Date.now() / 1000)) return null;
    return payload;
  } catch {
    return null;
  }
...
function isValidUsername(username: unknown): username is string {
  return typeof username === 'string' && USERNAME_RE.test(username.trim().toLowerCase());
}

function issueSession(user: { id: number; username: string; role: string }) {
  ...
  return { token: signSession(payload), user: { id: user.id, username: user.username, role } };
}
...
function requireAuth(req: AuthRequest, res: Response, next: NextFunction) {
  ...
  if (!auth) return res.status(401).json({ error: 'نیاز به ورود به سامانه دارید.' });
  req.auth = auth;
  return next();
}
...
function requireRole(...roles: UserRole[]) {
  return (req: AuthRequest, res: Response, next: NextFunction) => {
    if (!req.auth || !roles.includes(req.auth.role)) return res.status(403).json({ error: 'شما مجوز انجام این عملیات را ندارید.' });
    return next();
  };
}
...
// Authentication endpoints intentionally remain public; all data endpoints below require a valid server session.
router.get('/auth/account-count', async (_req, res) => {
  try {
    const result = await db.select({ count: sql<number>`count(*)` }).from(users);
    return res.json(Number(result[0]?.count ?? 0));
  } catch (error) {
...
router.post('/auth/first-admin', async (req, res) => {
  ...
  try {
    ...
    if (Number(count[0]?.count ?? 0) !== 0) return res.status(409).json({ error: 'حساب اولیه قبلاً ایجاد شده است.' });
    const salt = randomBytes(16).toString('base64');
    const result = await db.insert(users).values({ username, passwordHash: passwordHash(password, salt), salt, role: 'admin' }).returning({ id: users.id, username: users.username, role: users.role });
    const session = issueSession(result[0]);
    await addAuditLog({ action: 'create', entity: 'users', entityId: result[0].id, description: `ایجاد حساب مدیر اولیه: ${username}` });
...
router.post('/auth/login', async (req, res) => {
  ...
  try {
    ...
    const user = result[0];
    if (!user || !user.passwordHash || !user.salt || !USER_ROLES.has(user.role)) {
      await addAuditLog({ action: 'login_failed', entity: 'system', description: `ورود ناموفق برای نام کاربری: ${username}` });
      return res.status(401).json({ error: 'نام کاربری یا رمز عبور نادرست است.' });
    }
    const computed = passwordHash(password, user.salt);
    if (computed.length !== user.passwordHash.length || createHmac('sha256', AUTH_SECRET).update(computed).digest('hex') !== createHmac('sha256', AUTH_SECRET).update(user.passwordHash).digest('hex')) {
      await addAuditLog({ action: 'login_failed', entity: 'users', entityId: user.id, description: `ورود ناموفق برای کاربر: ${username}` });
      return res.status(401).json({ error: 'نام کاربری یا رمز عبور نادرست است.' });
    }
    await db.update(users).set({ lastLoginAt: new Date() }).where(eq(users.id, user.id));
    const session = issueSession({ id: user.id, username: user.username, role: user.role });
    await addAuditLog({ action: 'login', entity: 'users', entityId: user.id, description: `ورود موفق کاربر: ${user.username}` });
...
router.get('/auth/session', requireAuth, (req: AuthRequest, res) => {
  return res.json({ authenticated: true, user: req.auth });
});

router.use(requireAuth);

const roleForTable: Record<string, UserRole[]> = {
  customers: ['admin', 'manager', 'agent', 'accountant'],
  contracts: ['admin', 'manager', 'agent', 'accountant'],
  messageLogs: ['admin', 'manager', 'agent', 'accountant'],
  invoices: ['admin', 'manager', 'accountant'],
  payments: ['admin', 'manager', 'accountant'],
  auditLogs: ['admin', 'manager'],
  users: ['admin'],
};

const createCrudRoutes = (tableName: string, tableSchema: TableSchema) => {
  const roles = roleForTable[tableName] || ['admin'];
  router.get(`/${tableName}`, requireRole(...roles), async (_req, res) => {
    try {
  ...
  router.post(`/${tableName}`, requireRole(...roles), async (req, res) => {
    try {
      if (!req.body || typeof req.body !== 'object' || Array.isArray(req.body)) return res.status(400).json({ error: 'بدنه درخواست نامعتبر است.' });
      const result = await db.insert(tableSchema).values(req.body).returning();
      const created = result[0];
      await addAuditLog({ action: 'create', entity: tableName, entityId: created?.id, description: `Created record ID: ${created?.id ?? 'unknown'}`, after: created });
  ...
  router.put(`/${tableName}/:id`, requireRole(...roles), async (req, res) => {
    ...
    try {
      ...
      await addAuditLog({ action: 'update', entity: tableName, entityId: id, description: `Updated record ID: ${id}`, before: beforeResult[0], after: result[0] });
  ...
  router.delete(`/${tableName}/:id`, requireRole('admin'), async (req, res) => {
    ...
    try {
      ...
      await addAuditLog({ action: 'delete', entity: tableName, entityId: id, description: `Deleted record ID: ${id}`, before: beforeResult[0] });
      return res.json({ success: true });
    } catch (error) {
      return res.status(500).json(safeError(error));
    }
...
router.post('/settings', requireRole('admin'), async (req, res) => {
  try {
    if (!req.body || typeof req.body !== 'object' || Array.isArray(req.body)) return res.status(400).json({ error: 'تنظیمات نامعتبر است.' });
    const result = await db.insert(settings).values({ id: 1, data: req.body }).onConflictDoUpdate({ target: settings.id, set: { data: req.body } }).returning();
...
router.put('/settings/1', requireRole('admin'), async (req, res) => {
  try {
    if (!req.body || typeof req.body !== 'object' || Array.isArray(req.body)) return res.status(400).json({ error: 'تنظیمات نامعتبر است.' });
...
router.post('/contracts/complete', requireRole('admin', 'manager', 'agent', 'accountant'), async (req: AuthRequest, res) => {
  const { contract, invoice1, payment1, invoice2, payment2 } = req.body ?? {};
  if (!contract || typeof contract !== 'object') return res.status(400).json({ error: 'اطلاعات قرارداد الزامی است.' });
  if (!contract.contractNumber || typeof contract.contractNumber !== 'string' || !contract.contractNumber.trim()) return res.status(400).json({ error: 'شماره قرارداد الزامی است.' });
  try {
    const result = await db.transaction(async (tx) => {
      const existing = await tx.select({ id: contracts.id }).from(contracts).where(eq(contracts.contractNumber, contract.contractNumber.trim()));
    ...
    await addAuditLog({ action: 'create/complete', entity: 'contracts', entityId: result.contractId, description: `Completed contract ${contract.contractNumber}`, after: result });
    return res.status(201).json(result);
  } catch (error: any) {
    if (error?.message === 'DUPLICATE_CONTRACT_NUMBER') return res.status(409).json({ error: 'شماره قرارداد قبلاً ثبت شده است.' });
    return res.status(500).json(safeError(error));
  }
});
