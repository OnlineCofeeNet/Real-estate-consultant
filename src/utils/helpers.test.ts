...
describe('validateNationalId', () => {
  it('accepts a valid national ID', () => {
    // Known valid Iranian national ID pattern
...
describe('validatePhone', () => {
  ...
  it('rejects invalid phones', () => {
    assert.equal(validatePhone('0912345678'), false); // too short
    assert.equal(validatePhone('091234567890'), false); // too long
    assert.equal(validatePhone('08123456789'), false); // wrong prefix
    assert.equal(validatePhone('+989123456789'), false); // not normalized
...
describe('formatBankCard', () => {
  it('formats 16-digit card with dashes and Persian digits', () => {
    ...
    assert.ok(result.includes('۶')); // Persian digit
  });
});
