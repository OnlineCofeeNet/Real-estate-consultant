...
const RecoverPassword = ({ onBack }: { onBack: () => void }) => {
  ...
  const submitSmsRecovery = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      await recoverPassword(username, phone, '', '', newPassword, 'sms');
      toast.success('رمز عبور با موفقیت تغییر کرد.');
      onBack();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'بازیابی رمز عبور ناموفق بود.');
    } finally {
      setBusy(false);
    }
  };

  const submitQuestionsRecovery = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    setBusy(true);
    try {
      await recoverPassword(username, phone, a1, a2, newPassword, 'questions');
      toast.success('رمز عبور با موفقیت تغییر کرد.');
      onBack();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'بازیابی رمز عبور ناموفق بود.');
  ...
  return (
    <AuthShell title="بازیابی رمز عبور" subtitle="روش بازیابی را انتخاب کنید.">
      <div className="space-y-4">
        {step === 1 && (
        ...
        {step === 2 && method === 'questions' && (
          <form onSubmit={submitQuestionsRecovery} className="space-y-4">
            <p className="text-sm text-slate-600 bg-slate-50 p-4 rounded-xl">{q1}</p>
            <Field label="پاسخ سوال اول" value={a1} onChange={setA1} required />
            <p className="text-sm text-slate-600 bg-slate-50 p-4 rounded-xl">{q2}</p>
            <Field label="پاسخ سوال دوم" value={a2} onChange={setA2} required />
            <Field label="رمز عبور جدید" value={newPassword} onChange={setNewPassword} type="password" required />
            {error && <p className="text-sm text-red-600 bg-red-50 rounded-lg p-3">{error}</p>}
            <button disabled={busy} className="w-full rounded-xl bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white py-3 font-bold">{busy ? 'در حال تغییر...' : 'تغییر رمز عبور'}</button>
          </form>
        )}
        <button onClick={onBack} className="w-full text-sm font-medium text-slate-500 hover:text-slate-800">بازگشت به ورود</button>
      </div>
    </AuthShell>
  );
};
...
const Field = ({ label, value, onChange, type = 'text', required = false, autoComplete }: { label: string; value: string; onChange: (value: string) => void; type?: string; required?: boolean; autoComplete?: string }) => (
  <label className="block">
    <span className="block text-sm font-medium text-slate-700 mb-1">{label}</span>
    <input type={type} value={value} onChange={(e) => onChange(e.target.value)} required={required} autoComplete={autoComplete} className="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none focus:ring-2 focus:ring-emerald-500" />
  </label>
);
