...
import { LayoutDashboard, FileText, Users, Settings, HelpCircle, Sparkles, Landmark, UserCheck } from 'lucide-react';
    ...
    <div className="flex h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden" dir="rtl">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-slate-900 text-white shadow-lg z-20">
        ...
        <nav className="flex-1 flex flex-col py-4 gap-1 px-3">
          ...
          <NavItem to="/agents" icon={<UserCheck size={20} className="text-amber-400" />} label="مباشرین و مشاوران" />
          <NavItem to="/customers" icon={<Users size={20} />} label="مشتریان" />
          <NavItem to="/accounting" icon={<Landmark size={20} className="text-emerald-400" />} label="حسابداری و چک‌ها" />
          <NavItem to="/smart-matching" icon={<Sparkles size={20} className="text-purple-400" />} label="تطبیق هوشمند املاک" />
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <nav className="md:hidden fixed bottom-0 w-full bg-slate-900 shadow-[0_-1px_3px_rgba(0,0,0,0.3)] flex justify-around z-20">
          ...
          <NavItem to="/accounting" icon={<Landmark size={20} className="text-amber-400" />} label="حسابداری" />
          <NavItem to="/smart-matching" icon={<Sparkles size={20} className="text-purple-400" />} label="املاک" />
          <NavItem to="/customers" icon={<Users size={20} />} label="مشتریان" />
        </nav>
      </div>
    </div>
