...
const Customers = () => {
  ...
  const getCustomerStatus = (customer: Customer) => {
    ...
    const hasUncollectedCheque = Boolean(
      ...
      customerContracts.some(
        (c) =>
          ((c.party1?.phone === customer.phone ||
            c.party1?.nationalId === customer.nationalId) &&
            c.party1PaymentMethod === "cheque" &&
            c.status !== "completed") ||
          ((c.party2?.phone === customer.phone ||
            c.party2?.nationalId === customer.nationalId) &&
    ...
    const hasDebt = Boolean(
      ...
      customerContracts.some(
        (c) =>
          ((c.party1?.phone === customer.phone ||
            c.party1?.nationalId === customer.nationalId) &&
            c.party1PaymentMethod === "credit") ||
          ((c.party2?.phone === customer.phone ||
            c.party2?.nationalId === customer.nationalId) &&
            c.party2PaymentMethod === "credit"),
      ),
    );
    ...
    const isDuplicate = allCustomers.some(
      (c) =>
        ...
        (c.phone === cleanedPhone ||
          (c.nationalId &&
            cleanedNationalId &&
            c.nationalId === cleanedNationalId)),
    );
    ...
    reader.onload = async (evt) => {
      try {
        ...
        for (const row of data as any[]) {
          const fullName =
            row["نام و نام خانوادگی"] || row["نام"] || row["fullName"] || "";
          const phone = toEnglishDigits(
            row["موبایل"] || row["تلفن"] || row["phone"] || "",
          );
          const nationalId = toEnglishDigits(
            row["کد ملی"] || row["کدملی"] || row["nationalId"] || "",
          );
  ...
  const handleSendSms = async () => {
    ...
    if (successCount > 0) {
      toast.success(
        `ارسال پیامک پایان یافت. موفق: ${toPersianDigits(successCount)} شماره`,
      );
    } else {
  ...
  const handleSendMessage = async () => {
    ...
    const isBusinessCard =
      messageText === settings?.defaultMessages?.businessCard;
    const imageBase64 =
    ...
    if (successCount > 0) {
      toast.success(
        `ارسال پیام پایان یافت. موفق: ${toPersianDigits(successCount)} پلتفرم`,
      );
    } else {
      toast.error(
        "هیچ پیامی ارسال نشد. مخاطبان باید ابتدا در ربات استارت (/start) زده باشند یا شناسه عددی چت آن‌ها ثبت شده باشد.",
      );
    }
    setIsMessageModalOpen(false);
    setMessageText("");
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            امکان جستجوی هوشمند بر اساس نام، کد ملی یا شماره همراه
          </p>
        </div>
        <div className="flex gap-2 flex-wrap">
          {selectedCustomers.length > 0 && (
            ...
            >
              ...
              {toPersianDigits(selectedCustomers.length)})
            </button>
          )}
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 space-y-3">
        ...
        {search.trim() && (
          <div className="flex items-center justify-between text-xs text-slate-600 bg-slate-50 px-3 py-2 rounded-lg border border-slate-100">
            <span>
              نتایج مشابه یافت شده برای «
              <strong className="text-emerald-700">{search}</strong>»:
            </span>
                    ...
                    onChange={(e) => {
                      if (e.target.checked && customers) {
                        setSelectedCustomers(
                          customers.map((c) => c.id as number),
                        );
                      } else {
                        setSelectedCustomers([]);
                      }
              ...
              {customers?.length === 0 ? (
                ...
                customers?.map((customer) => {
                  ...
                  if (st.hasUncollectedCheque) {
                    rowColorClass =
                      "bg-red-50/40 hover:bg-red-50/70 border-r-4 border-r-red-500 transition-colors";
                  } else if (st.hasDebt) {
                    rowColorClass =
                      "bg-amber-50/40 hover:bg-amber-50/70 border-r-4 border-r-amber-500 transition-colors";
                  } else if (st.isBuyer) {
                    rowColorClass =
                      "bg-emerald-50/30 hover:bg-emerald-50/60 border-r-4 border-r-emerald-400 transition-colors";
                  } else if (st.isLandlord) {
                    rowColorClass =
                      "bg-slate-100/60 hover:bg-slate-200/50 border-r-4 border-r-slate-400 transition-colors";
                  }
                    ...
                    <tr key={customer.id} className={rowColorClass}>
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          {st.hasUncollectedCheque && (
                            <span
                              ...
                              title="دارای چک وصول‌نشده"
                            ></span>
                          )}
                          {st.hasDebt && !st.hasUncollectedCheque && (
                            <span
                              className="w-2 h-2 rounded-full bg-amber-500"
                              title="دارای بدهی مالی"
                            ></span>
                          )}
                        </div>
                      </td>
                      <td className="p-4">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {st.hasUncollectedCheque && (
                            <span
                              className="inline-flex items-center gap-1 bg-red-100 text-red-800 border border-red-300 font-bold px-2 py-0.5 rounded-md text-xs shadow-2xs animate-pulse"
                              title="چک وصول نشده"
                            >
                          ...
                          {st.hasDebt && (
                            <span
                              className="inline-flex items-center gap-1 bg-amber-100 text-amber-900 border border-amber-300 font-bold px-2 py-0.5 rounded-md text-xs shadow-2xs"
                              title="دارای بدهی مالی"
                            >
                              <CreditCard
                                size={13}
                              ...
                              بدهکار
                              {st.debtAmount
                                ? `: ${toPersianDigits(st.debtAmount.toLocaleString())} تومان`
                                : ""}
                            </span>
                          )}
                          ...
                          {st.isLandlord && (
                            <span
                              className="inline-flex items-center gap-1 bg-slate-200 text-slate-800 border border-slate-300 font-bold px-2 py-0.5 rounded-md text-xs shadow-2xs"
                              title="موجر (هایلایت توسی)"
                            >
                          ...
                          {st.isBuyer && (
                            <span
                              className="inline-flex items-center gap-1 bg-emerald-100 text-emerald-800 border border-emerald-300 font-bold px-2 py-0.5 rounded-md text-xs shadow-2xs"
                              title="خریدار (هایلایت سبز کمرنگ)"
                            >
                              <UserCheck
                                size={13}
                      ...
                      <td
                        className="p-4 text-slate-600 font-mono text-sm"
                        dir="ltr"
                      >
                        {toPersianDigits(customer.phone)}
                      </td>
                      <td className="p-4 text-slate-600 text-sm font-mono">
                        {toPersianDigits(customer.birthDate) || "-"}
                      </td>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    required
                    className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:ring-2 focus:ring-emerald-500 outline-none transition-all text-sm"
                ...
                <div>
                  ...
                  <input
                    type="text"
                    maxLength={10}
                    className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:ring-2 focus:ring-emerald-500 outline-none transition-all font-mono text-sm"
                ...
                <div>
                  ...
                  <input
                    ...
                    dir="ltr"
                    required
                    className="w-full border border-slate-200 bg-slate-50 rounded-xl p-3 focus:ring-2 focus:ring-emerald-500 outline-none transition-all font-mono text-right text-sm"
                    placeholder="0912..."
                  ...
                  <div className="w-full">
                    <DatePicker
                      ...
                      onChange={(date: any) => {
                        ...
                        if (sDate) {
                          try {
                            eDate = moment(sDate, "jYYYY/jMM/jDD")
                              .add(1, "jYear")
              ...
              <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200 space-y-3">
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-2">
                    نقش مشتری در معاملات:
                  </label>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {[
                      {
                        role: "موجر",
                        label: "موجر (هایلایت توسی)",
                        colorClass:
                          "bg-slate-100 text-slate-800 border-slate-300 peer-checked:bg-slate-700 peer-checked:text-white",
                      },
                      {
                        role: "خریدار",
                        label: "خریدار (سبز کمرنگ)",
                        colorClass:
                          "bg-emerald-50 text-emerald-800 border-emerald-300 peer-checked:bg-emerald-700 peer-checked:text-white",
                      },
                      {
                        role: "مستأجر",
                        label: "مستأجر",
                        colorClass:
                          "bg-blue-50 text-blue-800 border-blue-200 peer-checked:bg-blue-600 peer-checked:text-white",
                      },
                      {
                        role: "فروشنده",
                        label: "فروشنده",
                        colorClass:
                          "bg-purple-50 text-purple-800 border-purple-200 peer-checked:bg-purple-600 peer-checked:text-white",
                      },
                    ].map((item) => {
                      const isChecked = (formData.roles || []).includes(
                        item.role,
                      );
                      return (
                        <label
                          key={item.role}
                          className="flex items-center gap-2 p-2 border rounded-lg cursor-pointer bg-white hover:border-slate-400 transition-all text-xs font-medium"
                        >
                          <input
                            type="checkbox"
                            ...
                            onChange={(e) => {
                              ...
                              if (newRoles.includes("موجر"))
                                newType = "landlord";
                              else if (newRoles.includes("خریدار"))
                                newType = "buyer";
                              else if (newRoles.includes("مستأجر"))
                                newType = "tenant";
                              else if (newRoles.includes("فروشنده"))
                                newType = "seller";
                ...
                <div className="pt-2 border-t border-slate-200/80 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <label className="flex items-center gap-2.5 p-2.5 bg-red-50/70 border border-red-200 rounded-xl cursor-pointer hover:bg-red-50 transition-colors">
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          hasUncollectedCheque: e.target.checked,
                        })
                      }
                    />
                    <div className="text-xs">
                      <span className="font-bold text-red-700 block">
                        دارای چک وصول‌نشده (رنگ قرمز)
                      </span>
                      <span className="text-[10px] text-red-500">
                        علامت‌گذاری با رنگ قرمز در لیست مشتریان
                      </span>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center gap-2.5 p-2.5 bg-amber-50/70 border border-amber-200 rounded-xl cursor-pointer hover:bg-amber-50 transition-colors">
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, hasDebt: e.target.checked })
                      }
                    />
                    <div className="text-xs">
                      <span className="font-bold text-amber-800 block">
                        دارای بدهی مالی (رنگ نارنجی)
                      </span>
                      <span className="text-[10px] text-amber-600">
                        علامت‌گذاری با رنگ نارنجی در لیست مشتریان
                      </span>
                    </div>
                  </label>
                </div>
                ...
                {formData.hasDebt && (
                  <div className="pt-1 animate-in fade-in">
                    ...
                    <input
                      ...
                      value={
                        formData.debtAmount
                          ? toPersianDigits(formData.debtAmount.toString())
                          : ""
                      }
                      onChange={(e) => {
                        const val =
              ...
              <div className="flex items-center gap-2 py-2">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      autoSendMessages: e.target.checked,
                    })
                  }
                />
                <label
                ...
                >
                  ارسال خودکار پیام‌های موعد (تولد، اتمام قرارداد و یادآوری
                  اجاره)
                </label>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3 pt-2">
                <div>
                  ...
                  <input
                    type="text"
                    dir="ltr"
                    className="w-full border border-slate-200 bg-slate-50 rounded-xl p-2 text-left focus:ring-2 focus:ring-blue-500 outline-none transition-all text-xs font-mono"
                    value={formData.telegramId || ""}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        telegramId: toEnglishDigits(e.target.value),
                      })
                    }
                ...
                <div>
                  ...
                  <input
                    type="text"
                    dir="ltr"
                    className="w-full border border-slate-200 bg-slate-50 rounded-xl p-2 text-left focus:ring-2 focus:ring-orange-500 outline-none transition-all text-xs font-mono"
                    value={formData.rubikaId || ""}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        rubikaId: toEnglishDigits(e.target.value),
                      })
                    }
                ...
                <div>
                  ...
                  <input
                    type="text"
                    dir="ltr"
                    className="w-full border border-slate-200 bg-slate-50 rounded-xl p-2 text-left focus:ring-2 focus:ring-emerald-500 outline-none transition-all text-xs font-mono"
                    value={formData.baleId || ""}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        baleId: toEnglishDigits(e.target.value),
                      })
                    }
                    placeholder="123456789"
                  />
              ...
              <div className="flex gap-3 pt-6 border-t border-slate-100">
                <button
                  ...
                  className="flex-1 border border-slate-200 text-slate-600 rounded-xl py-3 hover:bg-slate-50 font-bold transition-colors"
                >
                  انصراف
                </button>
                <button
                  type="submit"
                  className="flex-1 bg-emerald-600 text-white rounded-xl py-3 hover:bg-emerald-700 font-bold shadow-sm shadow-emerald-600/20 transition-colors"
                >
                  ذخیره مشتری
                </button>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-xl border border-slate-200 animate-in zoom-in-95 max-h-[90vh] overflow-y-auto">
            <h3 className="text-xl font-bold mb-4 text-slate-800 flex items-center gap-2">
              ...
              ارسال پیام به مشتریان ({toPersianDigits(
            ...
            <div className="space-y-4">
              <div>
                <p className="text-xs font-medium text-blue-700 bg-blue-50 p-3 rounded-xl border border-blue-100 leading-relaxed">
                  نام مشتری به صورت خودکار در ابتدای پیام قرار می‌گیرد و اطلاعات
                  و آیدی‌های املاک مانند یک امضا در انتهای پیام درج خواهد شد.
                </p>
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5 flex justify-between items-center">
                  ...
                  <span
                    ...
                    onClick={() =>
                      setMessageText(
                        settings?.defaultMessages?.businessCard || "",
                      )
                    }
                  >
                    کارت ویزیت
                ...
                <div className="flex gap-2 mt-2 flex-wrap">
                  <button
                    onClick={() => {
                      const welcome =
                        settings?.defaultMessages?.welcome ||
                  ...
                  <button
                    onClick={() =>
                      setMessageText(settings?.defaultMessages?.birthday || "")
                    }
                    className="text-[11px] bg-slate-100 px-2.5 py-1 rounded-lg text-slate-600 hover:bg-slate-200"
                  >
                    تبریک تولد
                  </button>
                  <button
                    onClick={() =>
                      setMessageText(
                        settings?.defaultMessages?.contractExpiry || "",
                      )
                    }
                    className="text-[11px] bg-slate-100 px-2.5 py-1 rounded-lg text-slate-600 hover:bg-slate-200"
                  >
                  ...
                  <button
                    onClick={() =>
                      setMessageText(
                        settings?.defaultMessages?.rentPayment || "",
                      )
                    }
                    className="text-[11px] bg-slate-100 px-2.5 py-1 rounded-lg text-slate-600 hover:bg-slate-200"
                  >
                  ...
                  <button
                    onClick={() =>
                      setMessageText(settings?.defaultMessages?.chequeDue || "")
                    }
                    className="text-[11px] bg-slate-100 px-2.5 py-1 rounded-lg text-slate-600 hover:bg-slate-200"
                  >
                    وصول چک
              ...
              <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200">
                <div className="flex items-center gap-1.5 mb-1.5 text-emerald-700">
                  ...
                  <span className="text-xs font-bold">
                    امضای انتهای پیام (اطلاعات و آیدی‌های املاک):
                  </span>
                </div>
                <pre
                  className="text-[11px] text-slate-600 font-sans whitespace-pre-wrap leading-relaxed bg-white p-2.5 rounded-lg border border-slate-200"
                  dir="rtl"
                >
                  {getAgencySignature(settings) ||
                    "اطلاعات املاک جهت درج در امضا تنظیم نشده است."}
                </pre>
              </div>
              ...
              <div className="flex gap-3 pt-4 border-t border-slate-100">
                <button
                  ...
                  className="flex-1 border border-slate-200 text-slate-600 rounded-xl py-3 hover:bg-slate-50 font-bold transition-colors text-sm"
                >
                  انصراف
                </button>
                <button
                  onClick={handleSendMessage}
                  className="flex-1 bg-blue-600 text-white rounded-xl py-3 hover:bg-blue-700 font-bold shadow-sm shadow-blue-600/20 transition-colors flex justify-center items-center gap-2 text-sm"
                >
