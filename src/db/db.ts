...
class ApiDatabase {
  customers = new ApiTable<Customer>('/api/customers'); contracts = new ApiTable<Contract>('/api/contracts'); settings = new ApiTable<Settings>('/api/settings'); messageLogs = new ApiTable<MessageLog>('/api/messageLogs'); auditLogs = new ApiTable<AuditLog>('/api/auditLogs'); invoices = new ApiTable<Invoice>('/api/invoices'); payments = new ApiTable<Payment>('/api/payments'); users = new ApiTable<AuthUser>('/api/users'); properties = new ApiTable<Property>('/api/properties'); areas = new ApiTable<Area>('/api/areas');
  get tables() { return [
    { name: 'customers', ...this.customers }, { name: 'contracts', ...this.contracts }, { name: 'settings', ...this.settings }, { name: 'messageLogs', ...this.messageLogs }, { name: 'auditLogs', ...this.auditLogs }, { name: 'invoices', ...this.invoices }, { name: 'payments', ...this.payments }, { name: 'users', ...this.users }, { name: 'properties', ...this.properties }, { name: 'areas', ...this.areas }
  ]; }
...
export function useLiveQuery<T>(querier: () => Promise<T>, deps: any[] = []): T | undefined { const [data, setData] = useState<T | undefined>(undefined); useEffect(() => { let active = true; const fetch = () => { querier().then(res => { if (active) setData(res); }).catch(console.error); }; fetch(); listeners.add(fetch); return () => { active = false; listeners.delete(fetch); }; }, deps); return data; }
