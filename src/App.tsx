...
import { installSecurityActivityMonitor } from './services/security';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const cleanupSecurity = installSecurityActivityMonitor();
    return () => { document.removeEventListener('visibilitychange', handleVisibilityChange); window.removeEventListener('beforeunload', handleBeforeUnload); cleanupSecurity(); };
  }, []);
  ...
  return <BrowserRouter><Toaster position="top-center"/><Routes><Route element={<AuthGate/>}><Route element={<Layout/>}>
    ...
    <Route element={<ProtectedRoute permission="users"/>}><Route path="users" element={<UserManagement/>}/></Route>
