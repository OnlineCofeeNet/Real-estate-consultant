...
// PostgreSQL is the authoritative source. Automatic browser recovery must never overwrite it.
export const checkAndRestoreAutoBackup = async () => { try { const snapshot = await getSnapshot(); if (snapshot?.data) console.info('Browser backup snapshot available; automatic server restore is intentionally disabled.'); } catch (e) { console.warn('Browser backup recovery check failed', e); } };
